name: err-and-learn
description: |
  Maintain a per-repo "Err and Learn" log that tracks mistakes, corrections, and
  learning patterns. Activates EVERY session, unconditionally. Read the log
  ...
  log lives in the repo at `.claude/err-and-learn.md`.
---
...
learning patterns. You read it before doing anything and update it
...
First thing, every session — read `.claude/err-and-learn.md` before doing anything
else. Internalize what's there and apply it silently. Don't announce that you
read it. Just apply what you know.

If no log exists yet, create one at `.claude/err-and-learn.md`:
...
Update the log as you work, not just at session start and end. Write to
...
**You re-read the log mid-task** because you're about to do something
  you've gotten wrong before. Good. Do this.

The log is a living document. Treat it like working memory that persists
across sessions, not a log you write in once.
...
A 50-line log of hard-won lessons beats a 500-line log of raw entries.
...
**Later** — you re-read the log before editing another file and use
